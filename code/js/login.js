...
define([
  ...
  function loginVM() {
    ...
    self.accountLocked = ko.computed(function () {
      return (
        self.loginCount() == config.MAX_LOGIN_COUNT &&
        self.leftSeconds() != "-1"
      );
    });
    ...
    self.login_need_input_username = ko.observable();
    ...
    self.login_need_input_username(
      service.needUsername().login_need_input_username
    );
            ...
            if (config.LOGIN_SECURITY_SUPPORT) {
              self.checkLoginData(function () {
                if (self.loginCount() == config.MAX_LOGIN_COUNT) {
                  ...
                  if (data.errorType == "badUsername") {
                    showAlert(
                      {
                        msg: "username_error_left",
      ...
      loginLockTimer = setInterval(function () {
        service.getLoginData({}, function (data) {
          ...
          if (self.leftSeconds() != data.login_lock_time) {
            ...
            self.uiLoginTimer(
              self.uiLoginTimer() > 0 ? self.uiLoginTimer() - 1 : 0
            );
          }
        });
      }, 1000);
    ...
    function checkPinAfterLogin(loginStatus, data) {
      if (loginStatus.status == "loggedIn") {
        if (state == "modem_waitpin") {
          ...
          (state == "modem_waitpuk" || data.pinnumber == 0) &&
          data.puknumber != 0
        ) {
          return pageState.WAIT_PUK;
        } else if (
          (data.puknumber == 0 || state == "modem_sim_destroy") &&
          state != "modem_sim_undetected" &&
          state != "modem_undetected"
        ) {
    ...
    function loginAfterCheckPin(loginStatus, data) {
      if (loginStatus.status == "loggedIn") {
        ...
        if ($.inArray(state, config.TEMPORARY_MODEM_MAIN_STATE) != -1) {
          ...
          (state == "modem_waitpuk" || parseInt(data.pinnumber) === 0) &&
          parseInt(data.puknumber) != 0
        ) {
          return pageState.WAIT_PUK;
        } else if (
          (parseInt(data.puknumber) === 0 || state == "modem_sim_destroy") &&
          state != "modem_sim_undetected" &&
          state != "modem_undetected"
        ) {
          return pageState.PUK_LOCKED;
  ...
  function gotoLogin() {
    if (
      ...
      _.indexOf(config.GUEST_HASH, window.location.hash) == -1
    ) {
