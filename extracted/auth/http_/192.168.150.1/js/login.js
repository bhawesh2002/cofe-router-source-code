...
define([ 'jquery', 'knockout', 'config/config', 'service', 'underscore', 'config/menu', "logout"],
    function($, ko,config, service, _, menu, logout) {
        ...
        function loginVM() {
            ...
            self.login_need_input_username(service.needUsername().login_need_input_username);
                    ...
                    if (data.result) {
                        ...
                        if(config.LOGIN_SECURITY_SUPPORT){
                            self.checkLoginData(function(){
								...
								{
								showAlert({msg: 'username_error_left', params: [config.MAX_LOGIN_COUNT - self.loginCount()]}, function () {
																        setFocus();
														 });
								}
								else
