...
function (_, $, ko, config, service, chosen) {
  ...
  function groupSms(messages) {
    ...
    $.each(theSortedPeoples, function (s_i, sp) {
      ...
      for (var i = 0; i < peoples[people].length; i++) {
        ...
        if (peoples[people][i].tag == "4" && peoples[people][i].groupId == "") {
          // 单条草稿
          hasDraft = true;
        }
      }
  ...
  function dealPhoneBooks() {
    ...
    var groupIds = [];
    for (var k = 0; k < groupDrafts.length; k++) {
      // 将草稿做对象Map封装，供草稿组点击后的草稿分解
      ...
      var itemId = getLastNumber(
        groupDrafts[k].number,
        config.SMS_MATCH_LENGTH
      );
    ...
    for (var g in groupedDraftsObject) {
      // 处理列表显示的草稿信息
      ...
      draftItem.draftShowName =
        draftItem.draftShowName.length > len
          ? draftItem.draftShowName.substring(0, len) + "..."
          : draftItem.draftShowName;
    ...
    for (var i = 0; i < config.listMsgs.length; i++) {
      ...
      if ($.inArray(smsItem.itemId, tmp) == -1) {
        options.push(
          new Option(
            smsItem.number,
            getLastNumber(smsItem.number, config.SMS_MATCH_LENGTH)
          )
        );
  ...
  function showSmsListData() {
    ...
    cleanSmsList();
    $.tmpl("smsTableTmpl", { data: config.listMsgs })
      .translate()
      .appendTo("#smslist-table");
  ...
  function showMultiDraftListData() {
    ...
    $.tmpl("smsTableTmpl", { data: groupDraftItems })
      .translate()
  ...
  function changeShownMsgs() {
    ...
    $.tmpl("smsTableTmpl", { data: shownMsgsTmp })
      .translate()
      .appendTo("#smslist-table");
  ...
  deleteSelectClickHandler = function () {
    ...
    function renderAfterDelete(items) {
      ...
      $.each(nums, function (i, e) {
        $("#smslist-item-" + getLastNumber(e, config.SMS_MATCH_LENGTH))
          .hide()
          .remove();
      });
    ...
    function getIdsBySelectedIds() {
      ...
      $.each(selectedItem, function (i, e) {
        ...
        if (checkbox.attr("groupid")) {
          ...
          nums.push(
            getLastNumber(checkbox.attr("number"), config.SMS_MATCH_LENGTH)
          );
        }
      });
      ...
      $.each(config.dbMsgs, function (i, e) {
        if (
          $.inArray(getLastNumber(e.number, config.SMS_MATCH_LENGTH), nums) !=
            -1 &&
          (typeof e.groupId == "undefined" || _.isEmpty(e.groupId + ""))
        ) {
          resultIds.push(e.id);
          normalIds.push(e.id);
        } else if ($.inArray(e.groupId + "", groups) != -1) {
          //删除草稿组
          resultIds.push(e.id);
        }
      });
  ...
  chatCancelClickHandler = function () {
    if (config.CONTENT_MODIFIED.modified) {
      ...
      var selectedContact = syncSelectAndChosen(
        $("select#chosenUserSelect"),
        $(".search-choice", "#chosenUserSelect_chosen")
      );
    ...
    if (config.CONTENT_MODIFIED.modified) {
      ...
      if (config.CONTENT_MODIFIED.message == "sms_to_save_draft") {
        ...
        showConfirm(config.CONTENT_MODIFIED.message, {
          ...
          no: function () {
            var result = config.CONTENT_MODIFIED.callback.no(
  ...
  sendSmsClickHandler = function () {
    ...
    if (nums.length == 1) {
      config.currentChatObject = getLastNumber(
        nums[0],
        config.SMS_MATCH_LENGTH
      );
      showLoading("sending");
      ...
      showLoading(
        "sending",
        "<button id='sms_cancel_sending' onclick='cancelSending()' class='btn btn-primary'>" +
    ...
    sendSms = function () {
      ...
      // Check SMS capacity before attempting to send
      if (!hasCapability || smsCapability.nvUsed >= smsCapability.nvTotal) {
        hideLoading();
      ...
      service.sendSMS(
        ...
        function (data) {
          ...
          tryToDisableCheckAll(
            $("#smslist-checkAll", "#smsListForm"),
            $(".smslist-item", "#smslist-table").length
          );
          gotoBottom();
          if (i + 1 == nums.length) {
  ...
  function updateMsgList(msg, number, counter) {
    ...
    if (item && item.length > 0) {
      ...
      item
        ...
        .val(msg.id)
        .attr("id", "checkbox" + msg.id);
      var contentHtml = msg.content;
      if (msg.tag == "4") {
        contentHtml =
          '<span class="smslist-item-draft-flag colorRed" data-trans="draft"></span>: ' +
          contentHtml;
      }
  ...
  dealContent = function (content) {
    if (config.HAS_PHONEBOOK) {
      return HTMLEncode(content).replace(/(\d{3,})/g, function (word) {
        var r =
          (new Date().getTime() + "").substring(6) +
          (getRandomInt(1000) + 1000);
        return (
          "<a id='aNumber" +
          r +
          "' href='javascript:openPhoneBook(\"" +
          r +
          '", "' +
          word +
          "\")'>" +
  ...
  deleteSingleItemClickHandler = function (id, resendCallback) {
    ...
    function deleteTheSingleItem(id) {
      service.deleteMessage(
        ...
        function (data) {
          var target = $(".smslist-item-delete", "#talk-item-" + id).attr(
            "target"
          );
          $("#talk-item-" + id)
            .hide()
  ...
  getPeopleLatestMsg = function (number) {
    for (var j = 0; j < config.dbMsgs.length; j++) {
      if (
        !config.dbMsgs[j].groupId &&
        getLastNumber(config.dbMsgs[j].number, config.SMS_MATCH_LENGTH) ==
          getLastNumber(number, config.SMS_MATCH_LENGTH)
      ) {
  ...
  resendClickHandler = function (id) {
    ...
    var targetNumber = $("div.smslist-item-resend", "#talk-item-" + id).attr(
      "target"
    );
    ...
    disableBtn($("#btn-send", "#inputpanel"));
    // Double-check capacity after showing loading dialog
    if (!hasCapability || smsCapability.nvUsed >= smsCapability.nvTotal) {
      hideLoading();
      enableBtn($("#btn-send", "#inputpanel"));
      showAlert("sms_capacity_is_full_for_send");
    ...
    var getSMSReady = function () {
      service.getSMSReady({}, function (data) {
        if (data.sms_cmd_status_result == "2") {
          $("input:button", "#smsListForm .smslist-btns").attr(
            "disabled",
            "disabled"
          );
          hideLoading();
  ...
  function checkSimStatusForSend() {
    ...
    if (data.simStatus != "modem_init_complete") {
      disableBtn($("#btn-send"));
      $("#sendSmsErrorLi").html(
        '<span data-trans="no_sim_card_message">' +
          $.i18n.prop("no_sim_card_message") +
          "</span>"
      );
  ...
  function updateSmsCapabilityStatus(capabilityContainer, callback) {
    service.getSmsCapability({}, function (capability) {
      if (capabilityContainer != null) {
        capabilityContainer.text(
          "(" +
            (capability.nvUsed > capability.nvTotal
              ? capability.nvTotal
              : capability.nvUsed) +
            "/" +
            capability.nvTotal +
  ...
  bindingEvents = function () {
    ...
    $("#chat-input", "#smsChatRoom")
      ...
      .live("focusout", function () {
        ...
        if (
          $this.val() == "" ||
          $this.val() == $.i18n.prop("chat_input_placehoder")
        ) {
          $this
            .val($.i18n.prop("chat_input_placehoder"))
            .attr("data-trans", "chat_input_placehoder");
        }
        updateChatInputWordLength();
      })
      .live("keyup", function () {
    ...
    $("select.chosen-select-deselect", "#smsChatRoom")
      .die()
  ...
  getShowNameByNumber = function (num) {
    for (var i = config.phonebook.length; i > 0; i--) {
      if (
        getLastNumber(
          config.phonebook[i - 1].pbm_number,
          config.SMS_MATCH_LENGTH
        ) == getLastNumber(num, config.SMS_MATCH_LENGTH)
      ) {
  ...
  smsItemClickHandler = function (num) {
    ...
    for (var i = 0; i < ops.length; i++) {
      var n = ops[i];
      if (
        getLastNumber(n.value, config.SMS_MATCH_LENGTH) ==
        config.currentChatObject
      ) {
        num = n.value;
        numberExist = true;
        break;
    ...
    for (var i = config.dbMsgs.length - 1; i >= 0; i--) {
      ...
      if (
        getLastNumber(e.number, config.SMS_MATCH_LENGTH) ==
          config.currentChatObject &&
        _.isEmpty(e.groupId)
      ) {
        e.isNew = false;
      ...
      service.setSmsRead({ ids: ids }, function (data) {
        if (data.result) {
          $(
            ...
            .addClass("hide");
          $(
            "#smslist-item-" + getLastNumber(num, config.SMS_MATCH_LENGTH)
  ...
  forwardClickHandler = function (id) {
    ...
    var hasContent =
      ...
      content != "" &&
      content != $.i18n.prop("chat_input_placehoder");
    if (hasContent) {
      saveDraftAction({
        content: content,
        numbers: selectedContact,
  ...
  updateChatInputWordLength = function () {
    ...
    if (strValue.length + encodeType.extendLen > maxLength) {
      ...
      msgInputDom.value =
        strValue.substr(0, iInsertToStartLength) + strValue.substr(insertPos);
  ...
  function draftListener() {
    var content = $("#chat-input", "#smsChatRoom").val();
    if (hasCapability) {
      var selectedContact = getSelectValFromChosen(
        $(".search-choice", "#chosenUserSelect_chosen")
      );
      var noContactSelected = !selectedContact || selectedContact.length == 0;
      var hasContent =
          ...
          if (data.messages && data.messages.length > 0) {
            ...
            for (; i < data.messages.length; i++) {
              ...
              for (var k = 0; k < numbers.length; k++) {
                ...
                if (
                  getLastNumber(num, config.SMS_MATCH_LENGTH) ==
                  getLastNumber(msg.number, config.SMS_MATCH_LENGTH)
                ) {
                  //if (num.indexOf(msg.number) == 0) {
                  msg.number = num;
                }
              }
            ...
            if (theGroupId == "") {
              ...
              msg.draftShowName =
                draftShowName.length > len
                  ? draftShowName.substring(0, len) + "..."
                  : draftShowName;
              msg.hasDraft = true;
              msg.totalCount = i;
  ...
  function loadData() {
    if (
      ready &&
      !scrolling &&
      lastItemOffsetTop < $(window).scrollTop() + $(window).height() &&
  ...
  clearSearchKey = function () {
    ...
    $("#searchInput")
      .addClass("ko-grid-search-txt-default")
      .attr("data-trans", "search");
  };
  /**
   * 点击搜索输入框事件
  ...
  searchTextClick = function () {
    ...
    if (searchText.hasClass("ko-grid-search-txt-default")) {
      ...
      searchText.val("");
      searchText
        .removeClass("ko-grid-search-txt-default")
        .removeAttr("data-trans");
    }
  };
  /**
